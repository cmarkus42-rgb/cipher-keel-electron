...
| `agent.harness` | **Der Harness-Platz**: die Kennung des CLI-Adapters, der die Laufzeit des Presets übersteuert (`claude-code`, `kimi-code`). Leer ist die Vorgabe und heißt „das Preset entscheidet" — also genau das Verhalten von vor dieser Einstellung, wer nichts einstellt merkt nichts. **Übersteuert wird nur, wenn die Laufzeit des Presets ohnehin auf ein fremdes CLI zeigt**: ein Preset mit `runtime: keel-harness` ist eine Niveau-B-Zelle und bleibt unangetastet, sonst machte die Wahl „Kimi" jede Niveau-B-Gitterzelle kaputt. Gelesen beim Sitzungsstart (`wirkung: naechste-session`) — ein laufender Pane hat seinen Prozess schon gestartet. Ist der gewählte Harness nicht installiert, scheitert der Start **benannt** am `isAvailable()`-Riegel in `SESSION_CREATE`; einen stillen Rückfall auf das Preset gibt es hier nicht. Steht bewusst unter `agent` und nicht unter `modelle.zuordnung`: eine Zuordnung dort zielt auf einen Modell-Eintrag, ein Harness ist ein Adapter (`src/main/model/harness-platz.ts`) | ja — Settings-Fenster, Abschnitt „Harness"; die Oberfläche folgt dem Hauptprozess-Teil, Ansichtsmodell (`SettingsAnsicht.harnessPlatz`) und Schreibkanal (`settings:harness-setzen`) stehen | ja — Settings-Fenster |
...
| **Harness-Wahl** (`runtime` = `claude-cli-tmux` / `kimi-cli-tmux` / `keel-harness`) | `KNOWN_RUNTIMES` (`preset/schema.ts`), aufgelöst über `RUNTIME_TO_ADAPTER_ID` (`agent/registry.ts`); seit dem 2026-08-30 übersteuert durch den Harness-Platz `agent.harness` (`model/harness-platz.ts`, angewandt in `SESSION_CREATE`) | ja — Settings-Fenster, Abschnitt „Harness" | **ja, seit A3** — nicht am Preset, sondern am Platz: `agent.harness` wählt zwischen den fremden CLIs und lässt ein Preset der eigenen Schleife unangetastet. Der `runtime`-Wert eines Presets selbst bleibt unveränderlich (Folgephase), und eine Übersteuerung je Sitzung an der Launcher-Kachel ist bewusst nicht gebaut — Entwurf `docs/superpowers/specs/2026-08-30-a3-harness-platz-und-infoknoepfe-design.md` §5 |
