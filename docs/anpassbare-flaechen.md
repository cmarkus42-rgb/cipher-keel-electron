...
**Stand:** 2026-08-22 — der Netzfilter der Maschine benannt (er hält Erstkontakte und verfälscht jede Messung); neuer Zuordnungsplatz `rolle:rechercheur` (das Modell des Unterlaufs war
...
| **`TIEFEN`** (`kurz`: 1 Suche / 2 Seiten, `gruendlich`: 3 Suchen / 5 Seiten) | `src/main/harness/rechercheur.ts` (§3.4) | Wie viele Suchen und Seitenabrufe ein Unterlauf des Rechercheurs je Tiefe machen darf. Gezählt wird, was **hinausgegangen ist oder gleich wird**, nicht der Erfolg: ein Abruf, der das Haus verlassen hat und dann scheiterte, zählt (sonst wäre ein Ziel mit zuverlässigem HTTP 500 ein unbegrenzter Kanal nach draußen); ein Aufruf, der an der Eingabeprüfung stirbt, zählt **nicht** (er hat nichts getragen). Unterschieden wird über die `aufrufId` am `netz.ausgehend`. Ein noch nicht ausgeführter Aufruf zählt — das ist fail-closed und macht die Zählung rennfrei gegen mehrere Aufrufe eines Zuges. Bis 2026-08-22 zählte jede bloße Absicht, und in vier von zehn gemessenen Recherchen verbrannten dadurch Tippfehler des Modells das ganze Seitenbudget. Über der Grenze kommt eine **benannte** Absage, kein stiller Leerlauf | nein | nein — Konstante im Quelltext, Neubau nötig |
